...
class CategoryArticlesScreen extends StatefulWidget {
  ...
  const CategoryArticlesScreen({Key? key, required this.categoryId, required this.categoryName}) : super(key: key);
...
class _CategoryArticlesScreenState extends State<CategoryArticlesScreen> {
  ...
  void initState() {
    Future.delayed(Duration.zero).then((_) async {
      await Provider.of<CategoryArticlesScreenViewModel>(context, listen: false).fetchArticlesFromRemote(widget.categoryId);
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("${widget.categoryName} нийтлэлүүд", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600),),
        centerTitle: true,
      ),
      body: SafeArea(
        child: Consumer<CategoryArticlesScreenViewModel>(
          builder: (context, model, __) {
            if (model.state == ViewState.busy) {
              ...
              return RefreshIndicator(
                ...
                onRefresh: () => model.fetchArticlesFromRemote(widget.categoryId),
                    ...
                    if (model.articles.isEmpty) {
                      return Padding(
                        ...
                        child: SizedBox(
                          ...
                          child: Center(child: Text('Одоохондоо ${widget.categoryName.toUpperCase()} ангиллын нийтлэл ороогүй байна', style: const TextStyle(fontSize: 16),))),
                      );
                    } else {
                      return (index == model.articles.length) 
                        ? ElevatedButton(
                            ...
                            onPressed: () async {
                              await model.loadMoreArticlesAndSet(lastArticleId: model.articles.last.id!, categoryId: widget.categoryId);
                            },
                          )
                        : GestureDetector(
